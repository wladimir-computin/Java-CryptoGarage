...
import CryptoGarage.libcryptogarage.net.ConReceiver;
import CryptoGarage.libcryptogarage.net.NetCon_async;
import CryptoGarage.libcryptogarage.net.TCPCon_async;
import CryptoGarage.libcryptogarage.net.UDPCon_async;
...
public class CryptCon {
    ...
    public CryptCon(String pass, String ip){
        new CryptCon(pass, ip, false);
    }

    public CryptCon(String pass, String ip, boolean newinstance){
        this.crypt = Crypter.init(pass);
        challengeManager = ChallengeManager.instance();
        tcpCon_async = new TCPCon_async(ip, Config.tcp_port, newinstance);
        udpCon_async = new UDPCon_async(ip, Config.udp_port, newinstance);
    }
    ...
    public void discoverDevices(CryptConReceiver receiver){
        udpCon_async.sendMessage(MessageType.HELLO + ":::" + Config.command_discover, true, new ConReceiver() {

            @Override
            public void onResponseReceived(String responseData) {
                if(responseData.startsWith(MessageType.DATA.toString())){ Content out = new Content(MessageType.DATA, responseData.replace(MessageType.DATA.toString() + ":::", ""));
    ...
    private Message encryptToMessage(MessageType type, String data, String challenge_response_b64) {
        ...
        try {
            Message m = new Message();
            m.type = type;
            m.data_b64 = Base64.getEncoder().encodeToString(data.getBytes("UTF8"));
            m.challenge_request_b64 = Base64.getEncoder().encodeToString(crypt.getRandomChallenge());
